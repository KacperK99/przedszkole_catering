...
from .forms import RegisterForm, AccountAuthenticationForm, SkladnikCreate, DanieCreate, TypDaniaCreate, ZestawCreate, AnulowanoZamowienie, SkladnikiDaniaCreate, ZestawDanCreate, UzytkownikEdit
from catering_przedszkole.models import Uzytkownik, Zestaw, Zamowienie, Skladnik, Danie, Typ_dania, Zestaw_dan, Skladnik_dania
from catering_przedszkole.controller.controller import get_all_visable_sets, get_data_of_sets, get_set_by_id, get_data_of_sets_by_id, get_data_of_ingredients, get_user_orders, get_order_by_id, get_all_ingredients, get_ingridient_by_id, get_dish_by_id, get_all_dishes, get_all_types, get_type_by_id, get_all_sets, change_set_visability, get_all_users, change_user_permissions, get_user_by_id, get_user_orders_by_id, change_order_status, get_all_orders, get_ingridient_dish_by_id, get_all_ingridients_dishes, get_all_sets_dishes, get_set_dish_by_id
from django.http import HttpResponse
import json
...
def mainpage_view(request, *args, **kwargs):
    ...
    today = date.today()
    return render(request, "mainpage.html", {'sets':sets, 'data_set':data_set, 'today':today})
...
def zestaw_view(request, id_zestaw):
    ...
    return render(request, 'zestaw.html', {'singleset':singleset, 'data':data, 'ing':ingridients})
...
def zamow_view(request, id_zestaw, *args, **kwargs):
    ...
    if ((request.method == 'GET') and request.GET.get("numberInput")):
        ...
        zam = Zamowienie.objects.create(komentarz_zamowienia=com, zamawiajacy=request.user, ilosc_zestawow=num, zestaw=zestaw_instance)
    return render(request, 'zamow.html', {'zestaw_id':id_zestaw, 'zestaw':zestaw_instance})
...
def admin_skladnik_update(request, id_skladnik):
    ...
    try:
        ...
        return redirect('admin_skladniki')
    form = SkladnikCreate(request.POST or None, request.FILES or None, instance = ingridient)
    if form.is_valid():
       form.save()
       return redirect('admin_skladniki')
    return render(request, 'admin_skladnik_add.html', {'add_ingridient':form})
...
def admin_dania_view(request):
    ...
    if type_query != '' and type_query is not None:
        dishes = dishes.filter(typ_dania__nazwa_typu_dania__icontains=type_query.strip())
...
def admin_zamowienia_uzytkownika(request, id_uzytkownika):
    ...
    return render(request, 'admin_zamowienia_uzytkownika.html', {'user_orders':user_orders})
...
def admin_zmien_status_potwierdzenia(request, id_zamowienia):
    if request.user.is_anonymous==True:
        ...
        return redirect('mainpage')
    change_order_status(request, id_zamowienia)
...
def admin_zmien_status_anulowania(request, id_zamowienia):
    ...
    try:
        ...
        return redirect('admin_zamowienia_uzytkownika', id_uzytkownika=order.zamawiajacy.ID)
    form = AnulowanoZamowienie(request.POST or None, request.FILES or None, instance = order)
    if form.is_valid():
       form.save()
       return redirect('admin_zamowienia_uzytkownika', id_uzytkownika=order.zamawiajacy.ID)
    return render(request, 'admin_zmien_status_anulowania.html', {'form':form})
...
def admin_skladniki_dania_view(request):
    ...
    if ingridient_query != '' and ingridient_query is not None:
        skladniki_dania = skladniki_dania.filter(skladnik__nazwa_skladnika__icontains=ingridient_query.strip())
    if dish_query != '' and dish_query is not None:
        skladniki_dania = skladniki_dania.filter(danie__nazwa_dania__icontains=dish_query.strip())

    return render(request, 'admin_skladniki_dania.html', {'skladniki_dania':skladniki_dania})
...
def admin_skladniki_dania_create(request):
    ...
    return render(request, 'admin_skladniki_dania_add.html', {'add_skladnik_dania':form})
...
def admin_skladniki_dania_update(request, id_skladniki_dania):
    ...
    try:
        ...
        return redirect('admin_skladniki_dania')
    form = SkladnikiDaniaCreate(request.POST or None, request.FILES or None, instance = skladnik_danie)
    if form.is_valid():
       form.save()
       return redirect('admin_skladniki_dania')
    return render(request, 'admin_skladniki_dania_add.html', {'add_skladnik_dania':form})
...
def admin_dania_zestawy_view(request):
    ...
    if set_query != '' and set_query is not None:
        dania_zestawy = dania_zestawy.filter(zestaw__nazwa_zestawu__icontains=set_query.strip())
    if dish_query != '' and dish_query is not None:
        dania_zestawy = dania_zestawy.filter(danie__nazwa_dania__icontains=dish_query.strip())
...
def admin_dania_zestawy_update(request, id_dania_zestawy):
    ...
    try:
        ...
        return redirect('admin_dania_zestawy')
    form = ZestawDanCreate(request.POST or None, request.FILES or None, instance = zestaw_danie)
    if form.is_valid():
       form.save()
       return redirect('admin_dania_zestawy')
    return render(request, 'admin_dania_zestawy_add.html', {'add_danie_zestaw':form})
