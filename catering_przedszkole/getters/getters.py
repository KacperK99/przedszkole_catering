...
def get_user_orders(request):
    ...
    data = (
        Zamowienie.objects.all()
        .filter(zamawiajacy=user_id)
...
def get_user_orders_by_id(request, id_uzytkownika):
    data = (
        Zamowienie.objects.all()
        .filter(zamawiajacy=id_uzytkownika)
        .order_by("-data_zamowienia")
    )
    return data


def change_order_confirm_status(request, id_zamowienia):
...
def change_order_cancel_status(request, id_zamowienia):
    id_zamowienia = int(id_zamowienia)
    order = Zamowienie.objects.get(pk=id_zamowienia)
    if order.czy_anulowano == True:
        order.czy_anulowano = False
    else:
